...
#  NF America River near Auburn, Ca
siteNumber <- "11427000"
...
## ten year chunks to paste together later

startDate <- "1987-10-01"
endDate <- "1996-09-30"
dischargeUnit_WY88_WY96 <- readNWISuv(siteNumber, parameterCd, startDate, endDate, tz = "America/Los_Angeles")

startDate <- "1996-10-01"
endDate <- "2006-09-30"
dischargeUnit_WY97_WY06 <- readNWISuv(siteNumber, parameterCd, startDate, endDate, tz = "America/Los_Angeles")

startDate <- "2006-10-01"
endDate <- "2016-09-30"
dischargeUnit_WY07_WY16 <- readNWISuv(siteNumber, parameterCd, startDate, endDate, tz = "America/Los_Angeles")

startDate <- "2016-10-01"
endDate <- "2023-06-06"
dischargeUnit_WY17_6Jun2023 <- readNWISuv(siteNumber, parameterCd, startDate, endDate, tz = "America/Los_Angeles")


## paste together 
dischargeUnit_WY88_5June2023 <- rbind( dischargeUnit_WY88_WY96,  dischargeUnit_WY97_WY06,
                                       dischargeUnit_WY07_WY16, dischargeUnit_WY17_6Jun2023)

# delete chunks to keep tidy
rm(dischargeUnit_WY88_WY96,  dischargeUnit_WY97_WY06, dischargeUnit_WY07_WY16, dischargeUnit_WY17_6Jun2023)

dischargeUnit_WY88_5June2023 <- renameNWISColumns(dischargeUnit_WY88_5June2023)
...
df <-   dischargeUnit_WY88_5June2023 %>% transmute(usgsid = 11427000, dtmn = dateTime, cfs = Flow_Inst, Flow_Inst_cd, tz_cd) %>% mutate(
...
pribbon <-  ggplot() + 
  ...
  # showlegendfalse above is for matching legend alpha to plot alpha - https://github.com/tidyverse/ggplot2/issues/2529
   ...
   geom_line  (data = df_p_23,     aes(x=dowy_dcml, y=cfs, color = '2023 (very wet)')) +
 ...
 # geom_point  (data = df_p_23,     aes(x=dowy_dcml, y=cfs, color = "2017")) +
  ...
  ggtitle("North Fork American River flow, Auburn, Ca (1990-2023) (unregulated)  -  April toward baseflow\n 2023 Conditions (red) relative to 1990-2022") + dark_theme_gray() #+
...
ggsave("nfrkamrcn_Apr 1.png", height = 9, width = 16, units = "in", dpi = 300)
...
pribbon <-  ggplot() + 
  ...
  scale_linetype_manual(name = "",values = c(2, 2), 
                        guide = guide_legend(override.aes = list(color = c("blue")))) +
  scale_y_continuous(expand= c(0.02, 0.02)) + 
  scale_x_continuous(expand= c(0.02, 0.02), breaks = c(244, 251, 274, 305, 336 ), labels = c( "June 1", "June 9", "July 1", "Aug 1", "Sep 1")) +
  theme(plot.margin = unit(c(1,1,1,1), "in")) +  theme(legend.position = c(0.5, 0.2)) +
  ggtitle("North Fork American River flow, Auburn, Ca (1990-2023) (unregulated)  -  June toward baseflow\n 2023 Conditions (red) relative to 1990-2022") + dark_theme_gray() #+
#guides(color = guide_legend(override.aes = list(alpha = 1)))
...
#z <- ggplotly(pribbon)
#htmlwidgets::saveWidget(as_widget(z), "nf_auburn_ribbons_apr1.html")
ggsave("nfrkamrcn_June 1.png", height = 9, width = 16, units = "in", dpi = 300)
