## old - reads QME - obsolete as Kyle sends up to date CSVs - next version ReadKylesDBExport.r
...
##############################
######################## add in output from new version, but just using sjr_blo and leaving all traces from old version, as sjr_blo is the focus
sjr_blokrhff_co <- read_csv("sjr_blo_07212022.csv") %>% transmute(date_time = dmy_hm(date_time), value = cfs, unit = "cfs", dv = "sjr_blokrhff")
############################
#############################
...
###### rename from scenraio = `baseline` to `stakeholder alternative`
df_co <- rbind(k1_co, k2_co, sjr_blokrhff_co, outge_co, splprtcl_co, whtwtr_co,
               resinfl_co, reselev_co, resstor_co, temp_co) %>% mutate(scenario = "stakeholder alternative")
...
#############switch subtraction order as now there's no baseline, 
sjr_blokrhff_co <- sjr_blokrhff_co %>% transmute(value_bl = value)
sjr_blokrhff <- cbind(sjr_blokrhff_co, sjr_blokrhff_pa) %>% transmute(date_time, value = value_bl-value, unit, dv )
...
df_diff <- rbind(k1, k2, sjr_blokrhff, outge, splprtcl, whtwtr, resinfl, reselev, resstor, temp) %>% 
            ...
            mutate(scenario = "stakeholder alternative minus proposed action")
head(df_diff)
...
rm(list=ls()[! ls() %in% c("df","df_diff", "daily_dowy")])
head(df_diff)
tail(df_diff)
...
df$scenario <- factor(df$scenario, levels = c("proposed action", "stakeholder alternative"))
unique(df$scenario)
...
#########################################
################# July run PG&E implementation of Stephanie's idea
########################################
...
sjrbloaf_dailyvol <- sjrbloaf_dailyvol %>% pivot_wider(names_from = scenario, values_from = af_day) %>% mutate(scenariodiff =  `stakeholder alternative` - `proposed action`)
head(sjrbloaf_dailyvol)
source("fun_defs.r")
...
ggplot(sjrbloaf_dailyvol, aes(x = wy, y = dowy, fill = scenariodiff)) + geom_tile() +
  ...
  scale_y_continuous(expand = c(0.02,0.02),
                     ...
                     labels = c("Oct Begin", "Nov", "Dec", "Jan", "Feb", "Mar", "Apr","May","Jun","Jul","Aug", "Sep", "Sep end"),
  ...
  ggtitle("San Joaquin River below Kerckhoff Dam, release volume difference [Daily]\nStakeholder Alternative minus Proposed FERC Re-licensing Action (15-min output, 28 June '22 output)\ncapital = more obvious, lowercase = less obvious (see enclosed)")

ggsave("DailyVolumeKerckhoffReleaseDifference_stkhldralt.jpg", dpi = 300, width = 17, height = 11, units = "in") 
...
for(i in 1:length(diffevents$letter)) { 
  ...
  p <-  ts_plot(df %>% filter(dv == "sjr_blokrhff"),start,end) +
    ggtitle(paste0("Model Output Divergence Event: ", diffevents$letter[i], "        ", start, " to ", end, 
                   ", SJR below Kerckhoff, 15-min flow, 9 June '22 ResSim output")) + labs(y = "CFS (15-min avg)", sec.axis = dup_axis) +
  ...
  ggsave(paste0(diffevents$letter[i]," - ", diffevents$wy_sjwytf[i],"_", start, " to ", end, 
                "_stkhldralt.jpg"), dpi = 300, width = 22, height = 13, units = "in")
  z <- ggplotly(p)
