...
#, 
top <- ggplot(df %>% filter(dv == dvs[3], scenario == "stakeholder alternative") %>% mutate(scen = "0721 version"), aes(dowy_hr, wy, group = as.factor(wy), height = value, fill = value )) + 
  geom_ridgeline_gradient(scale = 0.00010, min_height = -3500, size = ridgelinethickness) + 
...
p <- top / diffbott + plot_annotation(title = paste0(dvs[3], ' - 15 min data')) + 
  ...
  ggsave( "FlowsBelowKerckhoff_07212022version.jpg", dpi = 300, width = 22, height = 13, units = "in") 
